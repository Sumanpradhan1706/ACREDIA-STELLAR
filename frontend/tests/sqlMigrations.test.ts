...
describe('database migration policy model', () => {
  it('keeps the consolidated schema free of permissive public policies', () => {
    ...
    expect(setup).not.toMatch(/CREATE POLICY\s+"Public can view credentials for verification"/i);
  ...
  it('defines the production RLS policy set', () => {
    expect(setup).toContain('DROP POLICY IF EXISTS "Public can view credentials for verification"');
